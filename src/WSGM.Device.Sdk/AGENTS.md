...
- **Static data, not injection.** The SDK defines a glyph profile, control map, and assets.
  ...
  place — and not a security control. A plugin is an assembly WSGM loads and runs in-process, so it
  already has WSGM's authority; do not add sanitizing, allowlisting, or re-serializing here
...
- Plugins never call VIIPER, never own WSGM's Steam Input lease, and never touch HidHide. The SDK
